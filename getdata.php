            ...
            if (!empty($row['username'])){
                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if (!$row = mysqli_fetch_assoc($resultl)){
                    	...
                    	$sql = "INSERT INTO users_logs (username, serialnumber, fingerprint_id, checkin_date, checkin_time,subject_id) VALUES (? ,?, ?, CURDATE(), CURTIME(), ?)";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
            ...
            if (!mysqli_stmt_prepare($result, $sql)) {
                ...
                if ($row = mysqli_fetch_assoc($resultl)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $sql = "INSERT INTO users (username , serialnumber, gender, fingerprint_id, fingerprint_select) VALUES (?, ?, ?, ?, 1)";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
